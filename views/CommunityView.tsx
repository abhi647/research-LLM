...
import { MessageSquare, ArrowBigUp, Share2, TrendingUp, Search, Hash, Users, Flame, Zap, MoreHorizontal, Filter, Globe, BookOpen } from 'lucide-react';
            ...
            <div className={`flex flex-col border-r border-pastel-gray bg-white/50 backdrop-blur-sm transition-all duration-300 ${isSplitView ? 'w-[70px] items-center py-4' : 'w-64 p-4'}`}>
                ...
                <div className="space-y-1 w-full">
                    {CHANNELS.map((channel) => (
                        <button key={channel.name} className={`flex items-center gap-3 w-full p-2 rounded-xl transition-colors ${channel.name === 'General' ? 'bg-slate-100 text-slate-900 font-medium' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'} ${isSplitView ? 'justify-center' : ''}`}>
